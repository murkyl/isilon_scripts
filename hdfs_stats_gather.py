...
cmdlines = {
    ...
    "protocol_by_op":
    "isi statistics protocol --protocols=hdfs --zero --long --output=TimeStamp,NumOps,In,InAvg,Out,OutAvg,TimeMin,TimeMax,TimeAvg,Op --totalby=Op --degraded --interval=%d",

    "protocol_by_node":
    "isi statistics protocol --protocols=hdfs --zero --long --output=TimeStamp,In,Out,Op --totalby=node --degraded --interval=%d",
    ...
    "client_connections":
    " isi statistics query current --nodes=all --stats=node.clientstats.active.hdfs,node.clientstats.connected.hdfs --degraded --interval=%d",
...
def main():
    ...
    if options.log_directory is None:
        ...
        prefix = "hdfs-perf-" + str(datetime.date.today()) + "-"
    ...
    # before starting commands reset isi_hdfs_d statistics
    try:
        subprocess.check_call(["/usr/bin/isi_for_array", "/bin/pkill -WINCH -f 'lw-container hdfs'"])
    except subprocess.CalledProcessError as e:
        print "Failed to reset isi_hdfs_d.log statistics!\n%s" % str(e)
    ...
    # after commands gather isi_hdfs_d statistics
    try:
        subprocess.check_call(["/usr/bin/isi_for_array", "/bin/pkill -INFO -f 'lw-container hdfs'"])
    except subprocess.CalledProcessError as e:
        print "Failed to get isi_hdfs_d.log statistics!\n%s" % str(e)
